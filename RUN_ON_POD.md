...
/persistent/tools/opensleep-diagnostic \
    ...
    --timeout-seconds 3 \
    --reset-subsystems \
    --json-output /persistent/opensleep-diagnostic-reset.json \
    --verbose \
    2>&1 | tee /persistent/opensleep-diagnostic-reset.log
```

`--reset-subsystems` runs exactly the four fixed register writes to `0x20` documented in
`SAFETY.md` (mirroring `opensleep::reset::ResetController`'s existing sequence) and nothing else.
